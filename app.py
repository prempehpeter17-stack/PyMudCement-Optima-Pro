...
# Initialize Async DB Schema on startup safely
asyncio.run(init_db())
...
# ==============================================================================
async def process_authentication(mode, username_val, password_val, email_val=None, company_val=None):
    async with AsyncSessionLocal() as session:
        if mode == "Register Account":
            try:
                # Optimized OR query check for existing username/email
                ...
                return False, "Registration failed due to duplicate entry constraints."
    ...
    with st.form("auth_form"):
        ...
        if submit:
            if not username or not password:
                ...
                success, response = asyncio.run(process_authentication(auth_mode, username, password, email, company))
                if auth_mode == "Register Account":
                    if success:
                        st.success(response)
                    else:
